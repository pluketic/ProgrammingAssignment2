...
makeCacheMatrix <- function(x = matrix()) {
    inverse_x <- NULL                                   # sets the value of local vector m to NULL (placeholder for inverse matrix)
    set<-function(y){                                   # define set() function which 
        x <<- y                                         # caches input matrix
        inverse_x <<- NULL                              # sets the value of cached variable m to NULL
    }
    ...
    getinverse<-function() inverse_x                    # define getmatrix() function which returns cached variable m (inverse of the input matrix)
...
## inverse matrix already exists in cache, if not then it calculates it.

cacheSolve <- function(x, ...) {
        ## Return a matrix that is the inverse of 'x'
    inverse_x <- x$getinverse()                         # define local vector m and assign it cached vector m (inverse of the input matrix)
    if(!is.null(inverse_x)){                            # if there is an cached inverse matrix m available in the cache
        print("getting cached inverse matrix")          # notify me about existing cached vector
        return(inverse_x)                               # and make function return cached vector m (inverse of the input matrix)
    } else {                                            # else
        ...
        inverse_x <- solve(matrix, ...)                 # define local variable m and assign it a value of inverse of the input matrix
